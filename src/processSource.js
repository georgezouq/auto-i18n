...
async function processSource(code = '') {
    ...
    pluginImportFromThis = globalData.pluginImportFromThis
    pluginImportPackage = globalData.pluginImportPackage
...
function _processSource(data = [], isTemplate = false) {
    ...
    while (index < len) {
        ...
        if (isTemplate) {
            result = extractTemplate(data[index], arr, index)
        } else {
            result = extractScript(data[index], arr, index)
        }
    ...
    if (pluginImportPackage && pluginImportPackage.length > 0 && hasModified) {
        data.splice(0, 0, pluginImportPackage)
    }
}
...
function replaceScript(str = '', data = []) {
    ...
    data.forEach(item => {
        ...
        const thisPart = pluginImportFromThis ? 'this.' : ''

        if (word == s) {
            s = `${thisPart}${pluginPrefix}('${replace}')`
        } else {
            // eslint-disable-next-line no-useless-concat
            s = '`' + s.replace(word, '${' + `${thisPart}${pluginPrefix}('${replace}')` + '}') + '`'
        }
